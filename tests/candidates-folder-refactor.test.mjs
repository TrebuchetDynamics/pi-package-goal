...
try {
  ...
  const scannerScript = path.join(root, "skills/engineering/candidates-folder-refactor/scripts/find-candidates.mjs");
  ...
  const proseGeneratedFixture = fs.mkdtempSync(path.join(os.tmpdir(), "autofolderrefactor-prose-generated-"));
  fs.mkdirSync(path.join(proseGeneratedFixture, "planning/triage"), { recursive: true });
  fs.mkdirSync(path.join(proseGeneratedFixture, "planning/goal"), { recursive: true });
  fs.mkdirSync(path.join(proseGeneratedFixture, "planning/handoff"), { recursive: true });
  fs.writeFileSync(path.join(proseGeneratedFixture, "planning/triage/SKILL.md"), "---\nname: triage\ndescription: Triage issues.\n---\n\n# Triage\n\n> This was generated by AI during triage.\n\nUse this as product documentation, not a generated artifact.\n");
  fs.writeFileSync(path.join(proseGeneratedFixture, "planning/goal/SKILL.md"), "---\nname: goal\ndescription: Goal discipline.\n---\n\n# Goal\n\nGenerated reports are evidence, not artifacts to ignore.\n");
  fs.writeFileSync(path.join(proseGeneratedFixture, "planning/handoff/SKILL.md"), "---\nname: handoff\ndescription: Summaries.\n---\n\n# Handoff\n\nDo not commit generated local logs, but keep this skill.\n");
  const proseGeneratedOutput = execFileSync(process.execPath, [scannerScript, ".", "--top", "5", "--no-log"], { cwd: proseGeneratedFixture, encoding: "utf8" });
  assert.doesNotMatch(proseGeneratedOutput, /(?:^|\n)(?:- .*planning\/|\d+\. planning\b)/, "prose mentioning generated AI must not suggest or rank product docs as generated/refactor noise");
  fs.rmSync(proseGeneratedFixture, { recursive: true, force: true });
