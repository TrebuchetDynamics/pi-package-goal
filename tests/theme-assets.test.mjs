...
const themePath = path.join(root, "themes", "goal-neon.json");
const theme = JSON.parse(fs.readFileSync(themePath, "utf8"));
assert.equal(theme.name, "goal-neon");
assert.ok(theme.$schema.includes("theme-schema.json"));
assert.ok(theme.vars.green);
for (const token of requiredColorTokens) {
  assert.ok(Object.hasOwn(theme.colors, token), `goal-neon theme missing color token: ${token}`);
}
