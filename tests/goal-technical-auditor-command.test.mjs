...
import { buildGoalTechnicalAuditorObjective, DEFAULT_TOKEN_BUDGET, formatScopeForObjective, parseGoalTechnicalAuditorArgs, validateScopeInsideCwd } from "../lib/goal-technical-auditor/command.js";

assert.equal(DEFAULT_TOKEN_BUDGET, "200k");
assert.deepEqual(parseGoalTechnicalAuditorArgs(""), { scope: ".", tokenBudget: "200k", dryRun: false, help: false, focus: null, error: null });
assert.deepEqual(parseGoalTechnicalAuditorArgs("skills"), { scope: "skills", tokenBudget: "200k", dryRun: false, help: false, focus: null, error: null });
assert.deepEqual(parseGoalTechnicalAuditorArgs("--tokens 500k extensions"), { scope: "extensions", tokenBudget: "500k", dryRun: false, help: false, focus: null, error: null });
assert.deepEqual(parseGoalTechnicalAuditorArgs("lib --tokens=1M"), { scope: "lib", tokenBudget: "1M", dryRun: false, help: false, focus: null, error: null });
assert.deepEqual(parseGoalTechnicalAuditorArgs("--tokens 0 extensions"), { scope: "extensions", tokenBudget: "200k", dryRun: false, help: false, focus: null, error: "Token budget must be positive." });
assert.deepEqual(parseGoalTechnicalAuditorArgs("--dry-run --tokens 500k extensions"), { scope: "extensions", tokenBudget: "500k", dryRun: true, help: false, focus: null, error: null });
assert.deepEqual(parseGoalTechnicalAuditorArgs("--focus bug-hunt-refactor lib"), { scope: "lib", tokenBudget: "200k", dryRun: false, help: false, focus: "bug-hunt-refactor", error: null });
