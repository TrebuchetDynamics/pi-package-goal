...
async function testExtensionLoadsAndRegistersCommands() {
  ...
  assert.match(extractedPrompt, /Task, constraints, validation, final JSON/);
  ...
  try {
    ...
    assert.match(sent[0].content, /Final contract \(JSON-only preferred; keep last\)/);
    assert.match(sent[0].content, /DEV_GOAL_REPORT: \{"validated":true\|false/);
    assert.match(sent[0].content, /blockedWork.*pivotedWorkCompleted/);
    assert.match(sent[0].content, /Rules: yes only after validation evidence/);
    assert.match(sent[0].content, /done=objective fully proven/);
    assert.match(sent[0].content, /Include blockedWork\+pivotedWorkCompleted in JSON/);
    assert.match(sent[0].content, /One repair-only retry/);
    assert.match(sent[0].content, /Lane direct; target 1200 tokens/);
    ...
    assert.match(sent[0].content, /Task, constraints, validation, final JSON/);
    ...
    assert.match(sent.at(-1).content, /Repair lane\. Target 700 tokens/);
    assert.match(sent.at(-1).content, /Do not edit code/);
    assert.match(sent.at(-1).content, /Do not change scope/);
    assert.match(sent.at(-1).content, /Do not run discovery/);
    assert.match(sent.at(-1).content, /Return only corrected DEV_GOAL_REPORT \+ markers/);
    ...
    assert.match(sent.at(-1).content, /Required schema:/);
    assert.match(sent.at(-1).content, /Previous JSON:/);
    assert.match(sent.at(-1).content, /"changedFiles":\["changed files"\]/);
    assert.ok(sent.at(-1).content.length <= 1400, `repair prompt should stay compact; got ${sent.at(-1).content.length}`);
    assert.equal(entries.at(-1).data.active, true);
    assert.equal(entries.at(-1).data.phase, "running");
