...
Report quality validator flags missing Blocked Work, missing Pivoted Work Completed, relative human-readable changed files, and vague DEV_GOAL_REPORT.changedFiles entries. A Final Report Gate logs compact state transitions with aggregate issue codes, gives malformed reports one informational repair-only retry with exact issue codes, then blocks as `malformed_final_report` if still invalid.
