...
- Run `/development-loop analyze-logs [path]` to summarize one log file or a directory of `logs.jsonl` files, including loop starts, iteration-result records, completion outcomes, unresolved starts, blocker reasons, postmortem causes/actions, self-improvement follow-ups, final-marker recovery requests/successes/blocks, delivery evidence, CI-green/CI-red and missing-gate records, empty provider responses, context overflows, compactions, topic sizes, repeated oversized topics, and likely improvement areas. Add `--html` to write a self-contained health report to the OS temp directory.
