...
export const ONKLAUD_USAGE = "Usage: /onklaud [--tokens 700k] [--dry-run] [status|install|task...]\nExamples: /onklaud status, /onklaud install --yes, /onklaud fix the failing tests, /onklaud";
...
function parsedArgs({ words = [], tokenBudget = DEFAULT_TOKEN_BUDGET, dryRun = false, help = false, yes = false, installDir = "", binDir = "", error = null } = {}) {
  ...
  const action = first === "status" || first === "install" ? first : "run";
...
export function onklaudCompletions(prefix = "") {
  return ["status", "install --yes", "install --dir ~/.local/share/onklaud-5 --bin-dir ~/.local/bin", "--dry-run", "--tokens 700k", "fix the failing tests", "improve this repo"]
