...
Catalog entries continue to record `theme: <name>` and skip `theme_axes` (the catalog's axes are looked up from [`tokens.css`](https://github.com/Nutlope/hallmark/blob/main/site/css/tokens.css)). Step 2.5 logic uses the same diversification check on both — for catalog entries it reads the axes from tokens.css; for custom entries it reads them from the entry.
...
2. **Does not save themes for reuse.** A custom run is per-output. The skill does not write back to [`tokens.css`](https://github.com/Nutlope/hallmark/blob/main/site/css/tokens.css). If the user wants a permanent theme, they paste the custom palette into tokens.css themselves and name it.
