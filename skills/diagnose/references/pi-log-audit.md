...
- current `ISSUE` reason with `failure_at=` for the last failure event, optional blocked-report `blocker=`, missing matching loop config such as `.pi/navivox-loop.json`, or malformed matching config JSON, plus `next_action=` guidance from logged `nextSteps` when present; historical failures in logs that later resumed, started a new compaction, or finished cleanly are reported as `HISTORY` without action guidance
