...
- current `ISSUE` reason with `failure_at=` for the last failure event, missing matching loop config such as `.pi/navivox-loop.json`, or malformed matching config JSON, plus `next_action=` guidance; historical failures in logs that later resumed or finished cleanly are reported as `HISTORY` without action guidance
