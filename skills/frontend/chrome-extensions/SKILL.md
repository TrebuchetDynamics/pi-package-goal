...
Build production-quality Chrome extensions using Manifest V3 and publish them to the Chrome Web Store. If `codebase-map-understand.md` exists and the extension task spans service workers, content scripts, popup/side-panel UI, or shared modules, consult the codebase map for relationship leads before broad manual exploration, then verify named files.
