...
3. Inspect `git status --short --branch`, repo instructions, existing maps (`codebase-map-understand.md` and `graphify-out/graph.json` when present), folder tree, imports/exports, callers, related tests, package manifests, and language module boundaries. Use Graphify queries for cross-folder caller/edge leads, then verify against live files.
...
4. **Continue autonomously**
   ...
   - If many files still sit in the old/root location and their destination is obvious from the topology, continue moving the next batch instead of reporting success. For root-file-heavy targets, root files are the primary debt: create focused responsibility subfolders/packages and leave only named entrypoints, facades, or compatibility files at root.
