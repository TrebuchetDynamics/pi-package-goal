...
- Uploads shorten the SAS URL through is.gd by default using HTTPS POST without requesting analytics, then try TinyURL if is.gd is unavailable. If the user requests no third-party shortening or the original URL, add `--no-shorten`.
