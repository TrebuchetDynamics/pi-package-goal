---
name: greploop
description: Iterate on an existing PR, MR, or shelved changelist until Greptile reaches 5/5 with no unresolved comments. Use only when that review context, tooling, auth, and external-action approval exist.
license: MIT
...
1. Read the file and understand the comment in context. If `codebase-map-understand.md` exists and the finding's impact is cross-module, consult the codebase map for caller/path leads and verify named files.
