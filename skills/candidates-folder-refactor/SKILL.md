...
Use `auto-folder-refactor N` only when the owner explicitly wants fully automatic candidate #1 → folder-refactor loops. The script expands the guarded folder-refactor prompt directly for Pi print mode, because slash-command injection can exit too early in non-interactive runs. It is scoped to the current working directory: scan roots and selected candidates must resolve to `pwd` or subfolders, never parents or symlink escapes. Automatic loops must refactor around behavior/responsibility with tests and clear module boundaries, not shallow file moves or vague `utils`/`common` buckets. Shared-code reuse and contract extraction are valuable only when they make the next change easier, safer, and more obvious without changing current behavior. When refactor candidates are exhausted or below `PI_AUTO_FOLDER_REFACTOR_BUGFIND_THRESHOLD`, it transitions to visibility-driven bug finding through small refactors, invariants, replay tests, and contract extraction.
