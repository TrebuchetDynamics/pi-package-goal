...
print_candidate_table() {
  C_BOLD="${bold}" C_DIM="${dim}" C_GREEN="${green}" C_YELLOW="${yellow}" C_CYAN="${cyan}" C_MAGENTA="${magenta}" C_RESET="${reset}" node -e '
    ...
    const rootPriority = allCandidates.filter((item) => item && item.relative && !skipped.has(item.relative) && (item.direct || 0) > 0)
      .sort((a, b) => depthOf(a) - depthOf(b) || (b.direct || 0) - (a.direct || 0) || (b.score || 0) - (a.score || 0) || (b.files || 0) - (a.files || 0) || a.relative.localeCompare(b.relative))[0];
