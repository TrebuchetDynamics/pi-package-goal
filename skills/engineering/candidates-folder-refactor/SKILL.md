...
Changed installs keep unique backups. Replacing an unowned non-empty install directory requires backups plus `AUTO_FOLDER_REFACTOR_INSTALL_FORCE=1` and is limited to paths inside `HOME`; symlink install directories are rejected.
