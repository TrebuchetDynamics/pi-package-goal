...
1. Inspect `git status --short --branch`, repo instructions, manifests, and the request-building code before editing. If `codebase-map-understand.md` exists and the harness spans multiple modules, consult the codebase map for request-builder/provider paths, then verify named files.
